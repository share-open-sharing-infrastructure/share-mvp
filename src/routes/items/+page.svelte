<script lang="ts">
	...
	import { Card, Img, Gallery, Button, Search, Dropdown, Checkbox } from 'flowbite-svelte';
			...
			<Dropdown simple class="w-48 p-2 text-sm">
				...
				{#each uniquePlaces as place}
					<li class="rounded p-1 hover:bg-gray-100 dark:hover:bg-gray-600">
						<Checkbox checked inline class="text-gray-900 focus:ring-gray-700" bind:group={selectedPlaces.selectedValues} value={place}>{place}</Checkbox>
					</li>
				{/each}
				<h6 class="mt-5 mb-1 ml-1 text-sm font-medium text-gray-900 dark:text-white">Name</h6>
				{#each uniqueNames as name}
					<li class="rounded p-1 hover:bg-gray-100 dark:hover:bg-gray-600">
						<Checkbox checked inline class="text-gray-900 focus:ring-gray-700" bind:group={selectedNames.selectedValues} value={name}>{name}</Checkbox>
					</li>
				{/each}
			</Dropdown>
		</TableHeader>
